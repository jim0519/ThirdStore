...
namespace ThirdStoreBusiness.ScheduleTask
{
    ...
    public partial class Task
    {
        ...
        public Task(T_ScheduleTask task)
        {
            this.Type = task.Type;
        ...
        private ITask CreateTask(ILifetimeScope scope)
        {
            ...
            if (this.Enabled)
            {
                ...
                {
                    object instance;
                    if (!ThirdStoreWebContext.Instance.TryResolve(type2, scope, out instance))
                    {
                        //not resolved
                        instance = ThirdStoreWebContext.Instance.ResolveUnregistered(type2, scope);
                    }
        ...
        public void Execute(bool throwException = false, bool dispose = true)
        {
            ...
            var scope = ThirdStoreWebContext.Instance.ContainerManager.Scope();
            var scheduleTaskService = ThirdStoreWebContext.Instance.Resolve<IScheduleTaskService>("", scope);
